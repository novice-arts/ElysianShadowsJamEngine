...
static inline bool (app_setup)(void){
    ...
    // Create window
    window = SDL_CreateWindow("SDL3 Window Example", 640, 480, SDL_WINDOW_RESIZABLE);
    if (window == NULL) {
...
static inline void (app_update)(void){
    ...
    SDL_SetRenderDrawColor(renderer, 0xFF, 0xFF, 0x00, 0xFF); // Red
...
static inline void (app_teardown)(void){
	...
	#define app_loop() emscripten_set_main_loop(app_loop, 0, 1);
	#define app_teardown() emscripten_cancel_main_loop(); (app_teardown)();
#else
	#define app_loop() do { (app_loop)(); }while(!quit)
#endif
